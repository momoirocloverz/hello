      ...
      <el-form @submit.native.prevent ref="form" :model="form" :rules="rule" label-width="150px">
        <el-row :gutter="15">
          <el-col :span="24">
            ...
            <div class="space"></div>
            <el-form-item label="集团发布范围：" prop="toUserIds">
...
export default class CouponDetail extends Vue {
  ...
  agentChange(ids: number[]) {
    this.form.toDealerCodes = ids;
  }
  ...
  async request() {
    ...
    if (this.form.toUserIds.length === 0 && this.form.toDealerCodes.length === 0) {
    ...
    try {
      ...
      form.toInternalAll = form.toInternalAll ? 1 : 0;
      ...
      let res = await api.post({ url: "COMPANY_PUBLISH_ARTICLE", isAdminApi: true, ...form });
