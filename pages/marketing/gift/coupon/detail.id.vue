<template>
  <div class="main-container">
    <breadcrumb-group :breadGroup="[{label:'优惠券',to:'/marketing/gift/coupon/index'},{label:'优惠券详情',to:''}]" />
    <el-card>
      <el-tabs>
        <el-tab-pane label="使用情况"
                     name="0">
          <statistics :info="statisticData"></statistics>
...
export default class couponDetail extends Vue {
  ...
  getDetail() {
    api.get({ url: "COUPON_BASEINFO", isAdminApi: true, id: this.pageId }).then((data: any) => {
      if (data.code === "000000") {
