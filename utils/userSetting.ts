 ...
 */
function filterUser(role: number | string = store.state.user.role): any {
  let _userItem: any = {};
...
class storeInfoSetting {
  ...
  public static clearInfo(): void {
    ...
    store.commit("user/setPermissionsKeys", []);
    store.commit("menu/set_permissRouterUrl", []);
    store.commit("menu/set_aside", []);
